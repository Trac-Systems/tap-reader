...
function bitLen(n) {
    let len;
    for (len = 0; n > _0n$8; n >>= _1n$6, len += 1)
        ;
    return len;
}
/**
...
const bitSet = (n, pos, value) => {
    return n | ((value ? _1n$6 : _0n$8) << BigInt(pos));
};
/**
 * Calculate mask for N bits. Not using ** operator with bigints because of old engines.
 * Same as BigInt(`0b${Array(i).fill('1').join('')}`)
 */
const bitMask = (n) => (_2n$3 << BigInt(n - 1)) - _1n$6;
// DRBG
...
// prettier-ignore
const _0n$7 = BigInt(0), _1n$5 = BigInt(1), _2n$2 = BigInt(2), _3n$2 = BigInt(3);
// prettier-ignore
...
function tonelliShanks(P) {
    ...
    // (a | p) ≡ 0    if a ≡ 0 (mod p)
    const legendreC = (P - _1n$5) / _2n$2;
    let Q, S, Z;
    // Step 1: By factoring out powers of 2 from p - 1,
    // find q and s such that p - 1 = q*(2^s) with q odd
    for (Q = P - _1n$5, S = 0; Q % _2n$2 === _0n$7; Q /= _2n$2, S++)
        ;
    // Step 2: Select a non-square z such that (z | p) ≡ -1 and set c ≡ zq
    for (Z = _2n$2; Z < P && pow(Z, legendreC, P) !== P - _1n$5; Z++)
        ;
    // Fast-path
    if (S === 1) {
        const p1div4 = (P + _1n$5) / _4n$1;
...
function hashToPrivateScalar(hash, groupOrder, isLE = false) {
    ...
    return mod(num, groupOrder - _1n$5) + _1n$5;
}
...
// prettier-ignore
const _0n$5 = BigInt(0), _1n$3 = BigInt(1); BigInt(2); const _3n$1 = BigInt(3); BigInt(4);
function weierstrassPoints(opts) {
    ...
    class Point {
        ...
        multiplyAndAddUnsafe(Q, a, b) {
            ...
            ) => (a === _0n$5 || a === _1n$3 || !P.equals(G) ? P.multiplyUnsafe(a) : P.multiply(a));
...
function taggedHash(tag, ...messages) {
    ...
    return sha256$2(concatBytes(tagP, ...messages));
}
...
function lift_x$1(x) {
    ...
    const p = new Point$1(x, y, _1n$2); // y(P) = y if y mod 2 = 0 or y(P) = p-y otherwise.
    p.assertValidity();
    return p;
}
/**
...
const u64 = {
    ...
    add: add$1, add3L, add3H, add4L, add4H, add5H, add5L,
};
var u64$1 = u64;
...
const sha512$1 = wrapConstructor$1(() => new SHA512());
...
function hash256(msg) {
    const b = Buff.bytes(msg);
    return Buff.raw(sha256$2(sha256$2(b)));
}
function hash160(msg) {
    const b = Buff.bytes(msg);
    return Buff.raw(ripemd160(sha256$2(b)));
}
function hmac256(key, msg) {
    const k = Buff.bytes(key);
    const b = Buff.bytes(msg);
    return Buff.raw(hmac$1(sha256$2, k, b));
}
...
const _3n = BigInt(3);
const _4n = BigInt(4);
const CONST = { N: N$1, P: P$1, G, _0n: _0n$2, _1n: _1n$1, _2n, _3n, _4n };
const ecc$1 = Field$1(N$1, 32, true);
...
function normalize_x(pubkey, xonly = false) {
    const key = Buff.bytes(pubkey);
    if (key.length === 32) {
...
function parse_extended_key(keydata) {
    const buffer = Buff.b58chk(keydata).stream;
    const data = {
        prefix: buffer.read(4).num,
        depth: buffer.read(1).num,
        fprint: buffer.read(4).num,
        index: buffer.read(4).num,
        code: buffer.read(32).hex,
        type: buffer.read(1).num,
...
var utils = /*#__PURE__*/Object.freeze({
    ...
    parse_extended_key: parse_extended_key,
    parse_x: parse_x,
    random: random
});
...
function on_curve(x, throws) {
    if (!(typeof x === 'bigint' && _0n$1 < x && x < P)) {
...
class Field extends Uint8Array {
    ...
    add(value) {
        ...
        const a = ecc$1.add(this.big, x.big);
        return new Field(a);
    }
    sub(value) {
        const x = Field.mod(value);
        const a = ecc$1.sub(this.big, x.big);
        return new Field(a);
    }
    mul(value) {
        const x = Field.mod(value);
        const a = ecc$1.mul(this.big, x.big);
        return new Field(a);
    }
    pow(value) {
        const x = Field.mod(value);
        const a = ecc$1.pow(this.big, x.big);
        return new Field(a);
    }
    div(value) {
        const x = Field.mod(value);
        const a = ecc$1.div(this.big, x.big);
        return new Field(a);
    }
    negate() {
...
class Point {
    ...
    static { this.G = new Point(CONST.G.x, CONST.G.y); }
...
function derive(key_path, key_data, chain_code, is_private = false) {
    valid_path(key_path);
    const is_m = key_path.startsWith('m');
    valid_chain(key_path, chain_code);
    ...
    for (const path of paths) {
        ...
        valid_derive_state(hardened, is_private);
        ...
        if (is_private) {
            ...
            in_field(key.big, true);
        }
        else {
            key = Point.from_x(key).add(tweak).buff;
            on_curve(key.slice(1).big);
        }
    }
    return [key, code];
}
function parse_path(fullpath) {
    const tweaks = [];
...
function get_shared_code(self_sec, peer_pub, options) {
...
function recover(signature, message, pub_key, rec_key) {
    ...
    const seed = get_shared_code(rec_key, pub_key, { tag: 'ecdh/recovery' });
...
function compute_nonce(secret, pubkey, message, options) {
    ...
    if (nonce !== undefined) {
        ...
        n = get_shared_code(secret, recovery, { tag: 'ecdh/recovery' });
    }
    else {
...
exports.code_config = code_config;
...
exports.noble = noble;
exports.point = point;
exports.ripe160 = ripe160;
exports.sha256 = sha256;
exports.sha512 = sha512;
exports.sign_config = sign_config;
